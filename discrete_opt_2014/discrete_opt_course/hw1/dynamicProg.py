...
def solve_it_via_rules(input_data):
    # Modify this code to run your optimization algorithm
...
def solve_via_grabbing_most_useful_items(capacity, items):
    ...
    print items   
    sortedItemList = sorted(items, key= lambda x : ((x.value*1.0)/x.weight)*1.0, reverse=True)
    print sortedItemList
 
    prevValue = -1
    for item in sortedItemList:
        if prevValue == -1:
            prevValue = (item.value*1.0)/item.weight
        curVal = (item.value*1.0)/item.weight
        print curVal
        if prevValue < curVal:
            print "ERROR!"
        else:
...
if __name__ == '__main__':
        ...
        print "My soloution"
        print solve_it_via_rules(input_data)
